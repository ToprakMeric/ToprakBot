...
public class GorunmezKarakter
{
    public static Tuple<string, string> Main(string ArticleText, string ArticleTitle)
    {
        ...
        if (luzumsuz1.Match(ArticleText).Success)
        { //Satır sonunda kırılmaz boşluk
        ...
        if (luzumsuz2.Match(ArticleText).Success)
        { //Satır başında kırılmaz boşluk
        ...
        if (luzumsuz3.Match(ArticleText).Success)
        { //Art arda kırılmaz boşluk-boşluk
        ...
        if (luzumsuz4.Match(ArticleText).Success)
        { //Başlık içinde kırılmaz boşluk
            bool bittimi = false;
            while (bittimi == false)
            {
                var cikar = luzumsuz4.Match(ArticleText);
                if (luzumsuz4.Match(ArticleText).Success)
                { //İlki kırılmaz boşluk
        ...
        if (luzumsuz5.Match(ArticleText).Success)
        { //Gereksiz zero-width space
        ...
        if (luzumsuz6.Match(ArticleText).Success)
        { //Gereksiz byte order mark
        ...
        if (luzumsuz7.Match(ArticleText).Success)
        { //combining dot above
            if (!(ArticleTitle == "Ayırıcı im" || ArticleTitle == "2021 Filistin-İsrail çatışmaları" || ArticleTitle == "Denesulinece" || ArticleTitle == "Hamilton mekaniği"))
            { //İstisna maddeler
        ...
        if (bulucu2.Match(ArticleText).Success)
        { //Zero-width space
        ...
        if (bulucu3.Match(ArticleText).Success)
        { //Thin space
        ...
        if (bulucu4.Match(ArticleText).Success)
        ...
        if (bulucu5.Match(ArticleText).Success)
        { //Ideographic space
        ...
        if (bulucu6.Match(ArticleText).Success)
        { //En space
        ...
        if (bulucu7.Match(ArticleText).Success)
        { //Hair space
        ...
        if (bulucu8.Match(ArticleText).Success)
        { //Byte order mark
        ...
        if (bulucu9.Match(ArticleText).Success)
        { //Punctuation space
        ...
        if (bulucu10.Match(ArticleText).Success)
        { //Zero-width joiner
        ...
        if (bulucu11.Match(ArticleText).Success)
        ...
        if (bulucu12.Match(ArticleText).Success)
        { //Soft hyphen	
