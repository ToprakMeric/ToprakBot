...
public class YalinURL {
    public static Tuple<string, string> Main(string ArticleText) {
        ...
        foreach(Match match in yalinurl.Matches(ArticleText))
            bos++;

        if(bos!=0&&!(sablonkontrol.Match(ArticleText).Success)&&yalinurl.Match(ArticleText).Success) {
            if(bos>=3)
                ArticleText="{{Yalın URL'leri temizle|tarih={{yk:CURRENTMONTHNAME}} {{yk:CURRENTYEAR}}}}\n"+ArticleText;
            else
                ArticleText=satirici.Replace(ArticleText, "$1$2 {{Satır içi yalın URL}}$3");
